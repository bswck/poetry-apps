...
# This script was adopted from https://github.com/bswck/skeleton/tree/1484ec4/project/scripts/bump.sh.jinja
#
# Usage:
# $ poe bump
...
# Automatically copied from https://github.com/bswck/skeleton/tree/1484ec4/handle-task-event.sh
make_token() {
    export TOKEN
...
ensure_gh_environment() {
    ...
    echo "$(jq -n '{"deployment_branch_policy": {"protected_branches": false, "custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/poetry-pipx/environments/$1" --input -)" > /dev/null 2>&1 || return 1
}
